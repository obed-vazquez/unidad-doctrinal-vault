...
- **`bugs-y-cambios.md`** — bitácora de iteraciones (`6a iteración`, `7a iteración`, …) para bugs y mejoras menores que no ameritan su propio archivo.
...
- **Bug o ajuste menor que no pertenece a ninguna feature documentada:** se agrega como entrada nueva en `bugs-y-cambios.md`.
...
Cuando una funcionalidad ya documentada cambia (nuevos requisitos, revisión de comportamiento después de entregada), se agrega una sección nueva al final del mismo archivo (por ejemplo `## Cambios posteriores` o `## Iteración N`), igual que hace `bugs-y-cambios.md` con sus iteraciones numeradas. No se reescribe ni se borra una decisión anterior en silencio: si una decisión nueva reemplaza una vieja, se dice explícitamente en la sección nueva.
